...
$internalToken = [Environment]::GetEnvironmentVariable('INTERNAL_API_TOKEN', 'Process')
if (-not $internalToken) {
  throw 'Missing INTERNAL_API_TOKEN in .env'
}

$nodeId = [Environment]::GetEnvironmentVariable('NODE_ID', 'Process')
if (-not $nodeId) {
  $nodeId = "local-node-$env:COMPUTERNAME"
  [Environment]::SetEnvironmentVariable('NODE_ID', $nodeId, 'Process')
}
...
Write-Host "[connector] NODE_ID=$nodeId"
