          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-2 scrollbar-thin scrollbar-thumb-gray-800">
            {nodes.map((node, i) => (
              <button
                ...
                className={`w-full text-left p-3 border rounded transition-all group ${status?.node?.name === node.name
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex flex-col gap-1 overflow-hidden">
                    <span className={`text-xs font-bold truncate ${status?.node?.name === node.name ? 'text-orange-400' : 'text-gray-400'}`}>
                      {node.name}
                    </span>
                    <span className="text-[9px] text-gray-700 font-mono">PROTO: {node.type?.toUpperCase() || 'UNKNOWN'}</span>
                  </div>
                  {status?.node?.name === node.name ? (
